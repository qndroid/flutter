...
class CropViewExtensions {
  ...
  static BitmapLoader resolveBitmapLoader(CropView cropView,
      CropView.Extensions.LoaderType loaderType) {
    switch (loaderType) {
      case PICASSO:
  ...
  static Rect computeTargetSize(int sourceWidth, int sourceHeight, int viewportWidth,
      int viewportHeight) {

    if (sourceWidth == viewportWidth && sourceHeight == viewportHeight) {
      return new Rect(0, 0, viewportWidth,
          viewportHeight); // Fail fast for when source matches exactly on viewport
    }
