...
List<Category> getAllGalleryDemoCategories(List<HomeModel> list) {
  String tempCategory;
  List<Category> result = <Category>[];
  list.forEach((HomeModel model) {
    if (tempCategory == null) {
      //空，直接添加进去
      result.add(model.category);
    } else {
      //不空判断是否添加过同类
      if (model.category.name != tempCategory) {
        result.add(model.category);
      }
    }
    tempCategory = model.category.name;
  });
  return result;
}

//将kAllGalleryDemos中的所有项，按GalleryDemoCategory目录分组，成为一个map
...
IconData getIconData(String iconName) {
  switch (iconName) {
    case 'animation':
      return GalleryIcons.animation;
    case 'category_mdc':
      return GalleryIcons.category_mdc;
    case 'phone_iphone':
      return GalleryIcons.phone_iphone;
    case 'drive_video':
      return GalleryIcons.drive_video;
  }
}
